...
import { createAudioFixerAgent } from './agent.js';
...
const audioFixerAgent = createAudioFixerAgent();

async function improve_for_tts(audio_to_improve) {
	try {
		const improved = await audioFixerAgent.improveAudio(audio_to_improve);
		return (improved || audio_to_improve).trim();
	} catch (e) {
		console.error('[TTS] Erro ao melhorar texto para TTS:', e.message);
		return audio_to_improve;
	}
}
...
client.on('message', async (msg) => {
	...
	if (msg.hasMedia && (msg.type === 'ptt' || msg.type === 'voice')) {
		isAudio = true;
		try {
